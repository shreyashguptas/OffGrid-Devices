...
// discriminates on `error.name === "Beacon2SoldOutError"` rather than instanceof.
class Beacon2SoldOutError extends Error {
  constructor(message = "Beacon 2 variant exists but is not currently available for sale.") {
    super(message);
    this.name = "Beacon2SoldOutError";
  }
}
 ...
 */
export async function verifyBeacon2Storefront(): Promise<void> {
  ...
  if (!product) {
    throw new Error(
      "Beacon 2 product was not found in Shopify (check the beacon-2 handle in src/lib/shopify-products.ts).",
    );
  }
