...
vi.mock("@/lib/shopify", () => ({
  ...
  createBeacon1CheckoutUrl: vi.fn(),
}));

import * as shopify from "@/lib/shopify";

describe("POST /api/shopify/beacon-1/checkout", () => {
  beforeEach(() => {
    vi.mocked(shopify.hasShopifyStorefrontConfig).mockReset();
    vi.mocked(shopify.createBeacon1CheckoutUrl).mockReset();
  });
  ...
  it("returns 200 with checkout URL when creation succeeds", async () => {
    ...
    vi.mocked(shopify.createBeacon1CheckoutUrl).mockResolvedValue(
  ...
  it("returns 500 when checkout creation throws", async () => {
    vi.mocked(shopify.hasShopifyStorefrontConfig).mockReturnValue(true);
    vi.mocked(shopify.createBeacon1CheckoutUrl).mockRejectedValue(
      new Error("sold out"),
    );
  ...
  it("rate limits repeated checkout creation attempts", async () => {
    ...
    const request = new Request("https://offgridevices.com/api/shopify/beacon-1/checkout", {
      method: "POST",
