...
vi.mock("@/lib/shopify", () => ({
  ...
  getBeacon1ProductWithCache: vi.fn(),
}));

import * as shopify from "@/lib/shopify";

describe("GET /api/shopify/beacon-1", () => {
  beforeEach(() => {
    vi.mocked(shopify.hasShopifyStorefrontConfig).mockReset();
    vi.mocked(shopify.getBeacon1ProductWithCache).mockReset();
  });
  ...
  it("returns 404 when product is missing", async () => {
    ...
    vi.mocked(shopify.getBeacon1ProductWithCache).mockResolvedValue(null);
  ...
  it("returns 200 with product when found", async () => {
    vi.mocked(shopify.hasShopifyStorefrontConfig).mockReturnValue(true);
    vi.mocked(shopify.getBeacon1ProductWithCache).mockResolvedValue({
      title: "Test",
      handle: "test",
