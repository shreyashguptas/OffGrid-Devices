...
4. **Phase 7 — Deploy gates** — GitHub branch protection recommended; **Vercel** `buildCommand` runs `verify:shopify` before `next build` so a broken Beacon 2 / checkout path **fails the deployment build**.
